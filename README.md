...
Single-command install via a Homebrew formula. It pulls in `openjdk@21` as a dependency,
...
[Homebrew on Linux](https://docs.brew.sh/Homebrew-on-Linux). The formula downloads the
same pre-built platform JAR as above and wraps it in a `conload` launcher script pinned
to Homebrew's `openjdk@21`.

The formula on `master` is regenerated by CI on every tagged release (new version +
sha256 checksums), so re-running the install command after a release upgrades you to
the latest version:

```bash
brew upgrade https://raw.githubusercontent.com/soft-am/conload/master/Formula/conload.rb
```

To uninstall:

```bash
brew uninstall conload
```
